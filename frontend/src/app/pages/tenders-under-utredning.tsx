...
export default function UnderUtredningPage() {
  ...
  useEffect(() => {
    const fetchTenders = async () => {
      try {
        setLoading(true);
        const data = await getTendersByState(TENDER_STATES.UNDER_UTREDNING);
        setTenders(data);
      } catch (err) {
      ...
      {tenders.length === 0 && (
        <div className="flex-grow flex items-center justify-center">
          <p className="text-lg text-gray-500">
            No tenders under review at the moment.
          </p>
        </div>
      )}
    </div>
...
const TenderCard = ({ tender }: { tender: Tender }) => {
  ...
  const handleCardClick = () => {
    ...
    router.push(`/dashboard/tenders/under-utredning/${encodedTenderName}`);
  };
